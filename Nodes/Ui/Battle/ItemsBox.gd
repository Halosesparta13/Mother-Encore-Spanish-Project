...
func enter(reset = false, _action = null):
	...
	if reset:
		...
		if itemList.size() > 0:
			updateInfoBox()
	if infoBox != null and not itemList.empty():
		infoBox.show()
...
func move(dir):
	...
	if not itemList.empty() and dir != Vector2.ZERO:
...
func select(idx):
	...
	if not globaldata.items.has(itemList[i].ItemName) or not doesItDoAnything(globaldata.items[itemList[i].ItemName]):
...
func updateInfoBox():
	if infoBox != null:
		...
		if not globaldata.items.has(item.ItemName):
			return 
...
func doesItDoAnything(itemData):
	if (itemData.HPrecover != 0 and itemData.boost["maxhp"] == 0) or (itemData.PPrecover != 0 and itemData.boost["maxpp"] == 0) or ("status_heals" in itemData and not itemData.status_heals.empty()) or ("battle_action" in itemData and itemData.battle_action != ""):
		return true
	else :
		return false
