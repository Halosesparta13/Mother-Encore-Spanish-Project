	...
	if visible and $PSISelect._active:
		...
		if event.is_action_pressed("ui_ctrl") and infoBox:
			if infoOnScreen:
				...
				$Tween.stop_all()
				$Tween.remove_all()
				$Tween.interpolate_property(infoBox, "rect_position:y", infoScreenPosition.y, infoScreenPosition.y + 60, 0.25, Tween.TRANS_EXPO, Tween.EASE_IN_OUT)
				$Tween.start()
			else :
				
				$Tween.stop_all()
				$Tween.remove_all()
				$Tween.interpolate_property(infoBox, "rect_position:y", infoBox.rect_position.y, infoScreenPosition.y, 0.25, Tween.TRANS_EXPO, Tween.EASE_IN_OUT)
				$Tween.start()
