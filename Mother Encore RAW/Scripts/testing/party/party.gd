...
func _physics_process(_delta):
	if global.partySpace[spacing] != null and active:
		...
		if not (abs(position.x - global.partySpace[spacing].x) > 2 or abs(position.y - global.partySpace[spacing].y) > 2):
			walk_type = FOLLOW
		...
		if walk_type == GO:
			...
			if position == global.partySpace[currentSpace]:
				currentSpace -= 1
			if not player.walk and not player.paused:
				currentSpace -= 1
		else :
		...
		if damaged == true and $DamageAnimation.get_current_animation() != "Flash" and not global.persistPlayer.paused:
			...
			if not partyMember["status"].has(globaldata.ailments.Unconscious) and partyMember["maxhp"] != 0:
	...
	for i in global.partyObjects:
		if i.climbing:
			canClimb = false
	if global.persistPlayer.state == global.persistPlayer.MOVE and not global.persistPlayer.paused and canClimb and globaldata.flags["switch_leader"] and global.party.size() != 1 and partyMemberClass == global.party:
...
func damage(Damage, DamageVariation = 0, _Hitdirect = Vector2.ZERO, Status = null):
	...
	if Status != null and not partyMember["status"].has(Status) and not partyMember["status"].has(globaldata.ailments.Unconscious):
