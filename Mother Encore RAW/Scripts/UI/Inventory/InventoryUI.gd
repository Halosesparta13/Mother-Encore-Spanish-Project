...
func _update_list():
	...
	if current_inventory == []:
		...
		for item in current_inventory:
			...
			if ((item_in_row > (selected_item_in_row - offset_list_threshold)) and offset_list) or not offset_list:
...
func _inputs():
	...
	if Input.is_action_just_pressed("ui_ctrl"):
		...
		if not is_inventory_empty and not swapmode and not sort_mode:
