...
func replace_npc(npc):
	...
	if not replaced.get_node("Shadow").visible:
...
func jump(height, length, times = 1, queue = false, Shadow = true, crouch = false):
	if queue:
		yield (self, "finished_action")
	blend_position(direction)
...
func blend_position(vector2):
	...
	if not replaced in global.partyObjects and not party_member and abs(vector2.x) - abs(vector2.y) == 0:
		blend = false
