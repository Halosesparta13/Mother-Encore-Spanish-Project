...
func _on_Area2D_body_entered(body):
	if touching == 0 and $Timer.time_left == 0 and unlocked == true and $AnimationPlayer.current_animation == "Normal" and not one_way:
		open()
	if one_way and global.persistPlayer.paused:
		unlock()
		open()
	if blocked and global.persistPlayer.running and global.persistPlayer.direction.y == - 1 and not unlocked:
...
func _on_Timer_timeout():
	if touching == 0:
		$AnimationPlayer.play("Normal")
		if end_sound != "" and not global.persistPlayer.paused:
...
func open():
	...
	if sound != "" and not global.persistPlayer.paused:
