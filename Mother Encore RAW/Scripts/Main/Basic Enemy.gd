...
func _physics_process(delta):
	if not global.persistPlayer.paused:
		animationTree.active = true
		...
		if eventRayCaster.get_collider() == global.persistPlayer and position.distance_to(start_pos) <= maxDistance:
			if state != CHASE and state != STUNNED:
				if (seeing == true or (global.persistPlayer.running == true and global.persistPlayer.substantialMovement == true)) and state != CHASE and not blind:
					start_chase()
			$Timer.wait_time = 2
			$Timer.start()
		else :
...
func move():
	...
	if not $RayCast2D.is_colliding() and (ample_distance_x > 8 or ample_distance_x < - 8 or ample_distance_y > 8 or ample_distance_y < - 8):
		newPos = travelPos
...
func start_chase():
	if (state == WANDER or state == RETURN) and not emotes.animaPlayer.is_playing() and not global.persistPlayer.paused:
		state = CHASE
		jump()
...
func _on_interact_body_entered(body):
	if visible and (body.name == "player" or "PartyFollower" in body.name) and not global.cutscene:
...
func _on_Hurtbox_area_entered(area):
	...
	if not global.persistPlayer.paused and not global.cutscene:
