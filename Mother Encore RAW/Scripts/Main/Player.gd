...
func _physics_process(delta):
	...
	if damaging == true and not paused:
		...
		if $DamageAnimation.get_current_animation() != "Flash":
			...
			if not global.party[0]["status"].has(globaldata.ailments.Unconscious):
...
func move_state(delta):
	if not paused:
		...
		_controls()
		_movement(delta)
		if Input.is_action_just_pressed("ui_cancel") and not climbing and not spinning and ((global.party[0] == globaldata.ninten and globaldata.flags["bat"]) or (global.party[0] == globaldata.lloyd and globaldata.flags["laser"]) or (global.party[0] == globaldata.ana)):
		...
		if Input.is_action_just_pressed("ui_ctrl") and not damaging:
		...
		if switching == true and spinning == true and spin_num == 4:
			...
			for i in global.partyObjects:
				if i != self:
					i.set_partyMember()
				i._spritesheet()
	if Input.is_action_just_pressed("ui_accept") and not paused and dialogue_box and eventRayCaster.is_colliding():
		if crouch:
			use_telepathy()
		else :
			interact_with()
	...
	if not paused:
		...
		if collide != null:
			if collide != eventCollider:
				...
				if not collide.is_connected("tree_exited", self, "set_eventCollider"):
...
func hit_stop(length, camShake, pause = false, timeScale = 0.0, animation = ""):
	...
	if length != 0 and not camera.shaking:
		if camShake != 0:
...
func damage(Damage, DamageVariation = 0, Hitdirect = Vector2.ZERO, Status = null):
	...
	hitdirect = Hitdirect
	if Status != null and not global.party[0]["status"].has(Status) and not global.party[0]["status"].has(globaldata.ailments.Unconscious):
...
func set_running(enabled):
	...
	if not paused:
		if not run_sound in runSoundStreams:
	...
	if $AudioStreamPlayer.playing and not enabled and not climbing:
		$AudioStreamPlayer.playing = false
