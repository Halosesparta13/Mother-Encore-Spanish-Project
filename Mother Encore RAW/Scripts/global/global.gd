...
func _physics_process(_delta):
	...
	if uiManager.uiStack.size() == 0:
		if Input.is_action_just_pressed("ui_select") and not persistPlayer.paused and persistPlayer.state == persistPlayer.MOVE:
...
func get_conscious_party():
	...
	for partyMember in party:
		if not partyMember["status"].has(globaldata.ailments.Unconscious):
