...
var limits_reached: = [false, false, false, false]
...
func _physics_process(delta):
	...
	if active:
		...
		if global.persistPlayer != null and update_player == true:
		...
		if marker_time >= 0.5:
			if $MapImage / MapMarkers.offset.y == 0:
		...
		if Input.is_action_just_pressed("ui_ctrl") and loaded_map != "404":
			$MapImage / MapMarkers.visible = not $MapImage / MapMarkers.visible
			$MapImage / PlayerMarker.visible = not $MapImage / PlayerMarker.visible
