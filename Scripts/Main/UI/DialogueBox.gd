...
func _process(delta):
	...
	if not dialogueLabel.visible_characters == len(dialogueLabel.text):
...
func _input(event):
	if not $AnimationPlayer.is_playing() and $WaitTimer.time_left == 0 and canInput:
...
func nextPhrase()->void :
	...
	if currPhrase.has("setpartymembers"):
		for i in currPhrase["setpartymembers"]:
			if globaldata.get(i) in global.party:
				if global.partyObjects.size() > 1 and not currPhrase["setpartymembers"][i]:
	...
	if currPhrase.has("setpartynpcs"):
		for i in currPhrase["setpartynpcs"]:
			if globaldata.get(i) in global.partyNpcs:
				if global.partyObjects.size() > 1 and not currPhrase["setpartynpcs"][i]:
	...
	if currPhrase.has("talker"):
		if not currPhrase["talker"] in ["null", "none", ""]:
	...
	if currPhrase.has("movecam"):
		...
		var time = 1.0
		if currPhrase["movecam"].has("actor"):
	...
	if currPhrase.has("name"):
		...
		if $Dialoguebox / Namebox.rect_position.y != - 15:
			$NameAnim.play("Open")
			if not currPhrase.has("cleardialog"):
				clear_dialogue()
				dialogueLabel.bbcode_text += "\n" + currPhrase["text"]
	...
	if currPhrase.has("heal"):
		...
		if character != "All" and character != "all":
			if not globaldata[character]["status"].has(globaldata.ailments.Unconscious):
	...
	if not currPhrase.has("text") and not currPhrase.has("wait"):
		next_dialog()
...
func endDialogue():
	...
	if actors.size() != 0:
		for i in actors:
			if not queued_battle:
				actors[i].update_npcs()
			elif not actors[i].drafted:
				actors[i].update_npcs()
			else :
...
func next_dialog():
	if currPhrase.has("if"):
		...
		for cond in currPhrase["if"]:
			...
			if "hasitem" in cond:
				if not InventoryManager.checkItemForAll(currPhrase["if"][cond]):
					condition = false
					if not currPhrase["if"].has("or"):
						break
			...
			if "hasstatus" in cond:
				...
				for member in global.party.size():
					if global.party[member]["name"] == character:
						if status != "":
							if not global.party[member]["status"].has(globaldata.ailments[str2var(status)]):
								condition = false
								hasStatus = false
								break
						else :
...
func show_box(show):
	...
	if not show and $Dialoguebox.rect_position.y != 180:
