...
export  var no_problem_thoughts = false
export (bool) var automaticShadow
export (bool) var no_shadow
export (bool) var no_collision
export  var sprite_offset = Vector2.ZERO
export  var initial_dir = Vector2.ZERO
export (String, "Idle", "Walk", "Talk") var idle_animation = "Idle"
export  var staring = false
export  var wander = false
export  var speed = 64
export  var walk_frequency = 2
...
func _physics_process(_delta):
	...
	if wander and newPos != null and not pause and not global.persistPlayer.paused and not global.inBattle and not global.queuedBattle:
		var difference = max(ceil(abs(speed / int(Engine.get_frames_per_second()))), 1)
		if not talking and (abs(newPos.x - position.x) > difference or abs(newPos.y - position.y) > difference) and not looking:
		...
		animationState.travel(idle_animation)
	if looking and not talking and (abs(old_Pos.x - position.x) < 1 or abs(old_Pos.y - position.y) < 1):
...
func _on_npc_tree_exiting():
	if not self in global.persistArray:
		$WanderRadius / Timer.stop()
